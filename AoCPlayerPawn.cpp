...
void AAoCPlayerPawn::ExecuteTerraformAction(int32 ActionIndex)
{
	...
	if (TerraForgeComp)
	{
		...
		// Use player feet position directly — bypasses camera raycast which may miss landscape
		const FVector PlayerPos = GetActorLocation();
		UE_LOG(LogTemp, Log, TEXT("TerraForge: Player action %s at (%.1f, %.1f, %.1f)"),
			*ActionName, PlayerPos.X, PlayerPos.Y, PlayerPos.Z);

		const bool bSuccess = TerraForgeComp->ExecuteActionAtLocation(PlayerPos, TFAction);
		if (bSuccess)
		{
