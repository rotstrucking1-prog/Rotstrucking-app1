...
public:
	...
	UFUNCTION(BlueprintCallable, Category = "AoC|NPC")
	void TakeDamageFromSource(float Damage, AActor* DamageInstigator);
	...
	UFUNCTION(BlueprintCallable, Category = "AoC|NPC")
	void LoadState(const FString& StateData);
...
private:
	...
	void TickHibernated(float DeltaTime);
	void TickBackground(float DeltaTime);
	void TickReduced(float DeltaTime);
	void TickFull(float DeltaTime);
	...
	// --- Oracle convenience methods ---
public:
	// --- Virtual stubs for overrides ---
