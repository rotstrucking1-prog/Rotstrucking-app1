...
void UAoCNPCSocialBrain::RecordInteraction(const FGuid& TargetID, const FString& TargetName, ESocialInteraction Type, float Magnitude)
{
...
void UAoCNPCSocialBrain::ApplyInteractionEffects(FNPCRelationship& Rel, ESocialInteraction Type, float Magnitude)
{
	...
	case ESocialInteraction::KilledMyAlly:
	...
	case ESocialInteraction::HelpedMe:
	case ESocialInteraction::HealedMe:
	case ESocialInteraction::RescuedMe:
		Rel.Trust += 15.f * Magnitude;
		Rel.Hostility -= 20.f * Magnitude;
		Rel.Familiarity += 10.f * Magnitude;
		break;

	case ESocialInteraction::GaveItem:
		Rel.Trust += 10.f * Magnitude;
		Rel.Hostility -= 8.f * Magnitude;
		Rel.Familiarity += 5.f * Magnitude;
		break;

	case ESocialInteraction::NearbyPresence:
...
void UAoCNPCSocialBrain::FormGroup(const TArray<AoCHumanoidNPCV2*>& InitialMembers)
{
	...
	for (AoCHumanoidNPCV2* Member : InitialMembers)
	{
		...
		RecordInteraction(Member->GetUniqueID(), Member->GetDisplayName(), ESocialInteraction::FightingTogether, 0.5f);
	}
...
void UAoCNPCSocialBrain::InitiateConversationWithNearest()
{
	// Stub: find nearest NPC and start conversation
}

void UAoCNPCSocialBrain::RequestPartyWithNearest()
{
	// Stub: request party formation with nearest NPC
}

void UAoCNPCSocialBrain::SetTraitValue(const FString& TraitName, float Value)
{
	// Stub: store personality trait
}
