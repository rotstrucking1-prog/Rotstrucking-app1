...
UENUM(BlueprintType)
enum class ESocialInteraction : uint8
{
	Greeting,
	Trading,
	GiftGiving,
...
struct FInteractionRecord
{
	...
	ESocialInteraction Type = ESocialInteraction::NearbyPresence;
...
public:
	...
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "AoC|NPC|Social")
	TArray<AActor*> PerceivedActors;

	int32 GetPerceivedNPCCount() const { return PerceivedActors.Num(); }

	UFUNCTION(BlueprintCallable, Category = "AoC|NPC|Social")
	void ModifyHostility(const FGuid& TargetID, float Delta);

	UFUNCTION(BlueprintCallable, Category = "AoC|NPC|Social")
	void RecordInteraction(const FGuid& TargetID, const FString& TargetName, ESocialInteraction Type, float Magnitude = 1.0f);
...
private:
	...
	void ApplyInteractionEffects(FNPCRelationship& Rel, ESocialInteraction Type, float Magnitude);
	...
	UFUNCTION(BlueprintCallable, Category = "AoC|NPC|Social")
public:
	void InitiateConversationWithNearest();

	UFUNCTION(BlueprintCallable, Category = "AoC|NPC|Social")
	void RequestPartyWithNearest();

	UFUNCTION(BlueprintCallable, Category = "AoC|NPC|Social")
	void SetTraitValue(const FString& TraitName, float Value);
