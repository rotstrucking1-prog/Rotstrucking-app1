...
void UAoCHUDWidget::AddToCanvas(UWidget* Widget, FVector2D Alignment, FVector2D Position, FVector2D Size, bool bAutoSize)
{
	...
	PanelSlot->SetAnchors(FAnchors(Alignment.X, Alignment.Y, Alignment.X, Alignment.Y));
...
void UAoCHUDWidget::BuildPlayerBars()
{
	...
	// HP
	{
		UProgressBar* TempBar = nullptr; UTextBlock* TempText = nullptr;
		UWidget* Row = MakeBarRow(TempBar, TempText, HPRed(), TEXT("100 / 100"), 22.f);
		HPBar = TempBar; HPText = TempText;
	...
	{
		UProgressBar* TempBar = nullptr; UTextBlock* TempText = nullptr;
		UWidget* Row = MakeBarRow(TempBar, TempText, MPBlue(), TEXT("50 / 50"), 20.f);
		MPBar = TempBar; MPText = TempText;
		UVerticalBoxSlot* S = PlayerBarsBox->AddChildToVerticalBox(Row);
		S->SetPadding(FMargin(0, 0, 0, 3));
	}
	// Stamina
	{
		UProgressBar* TempBar = nullptr; UTextBlock* TempText = nullptr;
		UWidget* Row = MakeBarRow(TempBar, TempText, StamGreen(), TEXT("100 / 100"), 18.f);
		StaminaBar = TempBar; StaminaText = TempText;
		UVerticalBoxSlot* S = PlayerBarsBox->AddChildToVerticalBox(Row);
		S->SetPadding(FMargin(0, 0, 0, 3));
	}
	// XP (thinner)
	{
		UProgressBar* TempBar = nullptr; UTextBlock* TempText = nullptr;
		UWidget* Row = MakeBarRow(TempBar, TempText, XPGold(), TEXT("0 / 1000"), 12.f);
		XPBar = TempBar; XPText = TempText;
		UVerticalBoxSlot* S = PlayerBarsBox->AddChildToVerticalBox(Row);
		S->SetPadding(FMargin(0, 0, 0, 3));
	}
	// Gold text
...
void UAoCHUDWidget::BuildChatBox()
{
	...
	// Input box
	{
		...
		FEditableTextBoxStyle InputStyle = ChatInput->WidgetStyle;
		...
		ChatInput->WidgetStyle = InputStyle;
...
void UAoCHUDWidget::BuildDeathScreen()
{
	...
	FButtonStyle BtnStyle = RespawnButton->WidgetStyle;
	...
	if (DeathSlot)
	{
		DeathSlot->SetAnchors(FAnchors(0, 0, 1, 1));
		DeathSlot->SetOffsets(FMargin(0, 0, 0, 0));
	}
