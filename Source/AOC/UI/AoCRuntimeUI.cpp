...
void UAoCRuntimeUI::NativeOnInitialized()
{
	...
	// Default to Game+UI so HUD buttons (chat tabs, hotbar, etc.) are clickable
	// Cursor stays hidden — players right-click-hold or press a key to show cursor
	if (APlayerController* PC = GetOwningPlayer())
	{
		...
		PC->SetShowMouseCursor(false);  // Cursor hidden by default, ESC shows it
	}
}
...
void UAoCRuntimeUI::ToggleGUIMode()
{
	...
	if (PC)
	{
		...
		{
			// Hide cursor, but keep Game+UI so HUD buttons still work
