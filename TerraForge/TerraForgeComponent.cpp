...
bool UTerraForgeComponent::BeginEditSession(float Radius)
{
	if (!TerraForgeSubsystem) return false;
	...
	return TerraForgeSubsystem->BeginEditSession(Center, Radius);
}

void UTerraForgeComponent::EndEditSession()
{
	if (!TerraForgeSubsystem) return;
	TerraForgeSubsystem->EndEditSession();
}
