...
class CombatBrain:
    ...
    def run(self):
        ...
        self.log(f"🔮 Mage:  {self.player.mage_weapon} ({WEAPON_DB.get(self.player.mage_weapon, {}).get('speed', '?')}t)")
        self.log(f"💥 Spec:  {self.player.spec_weapon} (max hit: {self.player.max_hit_with_spec()})")
        ...
        while self.fighting:
            try:
                ...
                self.combat_tick()
                # Aim for ~60 reads/second (every ~16ms)
                # But align major actions to game ticks
                elapsed = time.time() - loop_start
                sleep_time = max(0.010, 0.016 - elapsed)
                # Add micro-variation (anti-detection)
...
class BotGUI:
    ...
    def start_bot(self):
        if self.running:
            return
        if not self.brain.screen.find_window():
            self.brain.log("❌ Can't find Roat Pkz window!")
            return
        self.running = True
        self.brain.log("✅ Bot armed — press TARGET LOCK or F9 to engage")
        self.status_label.config(text="ARMED — waiting for target lock", foreground="yellow")
    ...
    def target_lock(self):
        """THE BUTTON — one click, target dies."""
        if not self.running:
            self.start_bot()
    ...
    def update_display(self):
        ...
        try:
            ...
            if self.brain.target.locked:
                ...
                self.action_label.config(text="Press TARGET LOCK or F9 to engage")
