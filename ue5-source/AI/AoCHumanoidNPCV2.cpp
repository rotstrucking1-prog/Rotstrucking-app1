...
AoCHumanoidNPCV2::AoCHumanoidNPCV2()
{
	...
	CurrentLODTier = EAILODTier::Full;
...
void AoCHumanoidNPCV2::GenerateName()
{
	...
	NameRand.Initialize(GetUniqueID());
	...
	{
		NPCName = FString::Printf(TEXT("NPC_%d"), GetUniqueID());
	}
}
...
void AoCHumanoidNPCV2::MasterTick(float DeltaTime)
{
	...
	if (World)
	{
		...
		if (LODMgr)
		{
			CurrentLODTier = LODMgr->GetLODTier(this);
		}
	}
...
void AoCHumanoidNPCV2::TakeDamageFromSource(float Damage, AActor* Instigator)
{
	if (bIsDead) return;
	...
	if (CombatBrain)
	{
		CombatBrain->OnDamageTaken(Damage, Instigator);

		// Enter combat if not already
		if (!CombatBrain->IsInCombat() && Instigator)
		{
			CombatBrain->EnterCombat(Instigator);
		}
	}
	...
	if (Memory)
	{
		// Memory->Remember(TEXT("Attacker"), GetActorLocation(), Damage, Instigator);
	}

	// Social: record being attacked
	if (SocialBrain && Instigator)
	{
		AoCHumanoidNPCV2* AttackerNPC = Cast<AoCHumanoidNPCV2>(Instigator);
		if (AttackerNPC)
		{
			SocialBrain->RecordInteraction(
				AttackerNPC->GetUniqueID(),
				AttackerNPC->GetDisplayName(),
				EInteractionType::AttackedMe,
...
void AoCHumanoidNPCV2::OnDeath(AActor* Killer)
{
	...
	if (bIsEssential || RespawnDelay > 0.f)
	{
		...
		GetWorldTimerManager().SetTimer(RespawnTimer, this, &AoCHumanoidNPCV2::OnRespawn, RespawnDelay, false);
	}
}
...
int32 AoCHumanoidNPCV2::GetLevel() const
{
	if (!Skills) return 1;
...
void AoCHumanoidNPCV2::LoadState(const FString& StateData)
{
	...
	if (Skills)
	{
		Skills->DeserializeState(StateData);
	}
